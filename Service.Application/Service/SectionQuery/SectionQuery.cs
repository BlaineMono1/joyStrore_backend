...
namespace Service.Application.Service.SectionQuery
{
    public class SectionQuery
    {
        ...
        public SectionQuery(
            ILogger<SectionQuery> logger,
            IRepository<Section> sectionRepository,
            IProductRepository<Product> productRepository,
            IRepository<SectionsProducts> sectionsEditionsRepository,
        ...
        public async Task CreateSections(string sectionName, string imagePath)
        {
            var section = new Section
            {
                ...
                Products = new List<SectionsProducts>(),
            };
        ...
        {
            var section = (await _sectionRepository.GetListQuery())
                .Include(s => s.Products)
                .FirstOrDefault(s => s.Guid == SectionId);
        ...
        public async Task<List<SectionsDto>> SectionsList()
        {
            ...
            var result = sections
                ...
                .Select(item => new SectionsDto { SectionId = item.Guid, SectionName = item.Name })
                .ToList();
        ...
        public async Task AddProductInSection(Guid SectionId, Guid ProductId)
        {
            ...
            var q = new SectionsProducts { ProductId = ProductId, SectionId = SectionId };
        ...
        {
            var delete = (await _sectionsEditionsRepository.GetListQuery()).FirstOrDefault(se =>
                se.SectionId == SectionId && se.ProductId == ProductId
            );

            if (delete is null)
                throw new NotFoundException(nameof(SectionsProducts), SectionId);
        ...
        public async Task<List<ProductDto>> FindProductByName(string Name, bool isEdition)
        {
            ...
            if (isEdition)
            {
                var products = (await _productRepository.GetListQuery())
                    .Include(p => p.Edition)
                    .Where(p => p.Type == "Game" && p.Edition.Name.Contains(Name));
                result = products
            ...
            else
            {
                var products = (await _productRepository.GetListQuery())
                    .Include(p => p.AddOn)
                    .Where(p => p.Type == "AddOn" && p.AddOn.Name.Contains(Name));
                result = products
                    .Select(item => new ProductDto
                    {
                        ProductId = item.Guid,
        ...
        public async Task UpdateProduct(Guid ProductId, string Name, string ImagePath)
        {
            ...
            if (product.Type == "Game")
            {
                ...
                if (edition is null)
                    throw new NotFoundException(nameof(Edition), product.TypeId);

                if (!string.IsNullOrEmpty(Name))
                    edition.Name = Name;

                if (!string.IsNullOrEmpty(ImagePath))
                    edition.Image = ImagePath;
            ...
            else if (product.Type == "AddOn")
            {
                ...
                if (addOn is null)
                    throw new NotFoundException(nameof(AddOn), product.TypeId);
        ...
        public async Task<List<AddOnSectionList>> GetAddOnsGroups()
        {
            ...
            result.AddRange(
                lst.Select(item => new AddOnSectionList { GroupId = item.Guid, Name = item.Name })
            );
        ...
        {
            var g =
                (await _groupAddOnRepository.GetListQuery())
                    .Include(gr => gr.AddOns)
                    .FirstOrDefault(gr => gr.Guid == GroupId)
        ...
        {
            var g =
                await _groupAddOnRepository.GetById(GroupId)
                ?? throw new NotFoundException(nameof(GroupAddOn), GroupId);
        ...
        public async Task<List<AddOnsLst>> AddOnsInGroup(Guid GroupId)
        {
            ...
            result.AddRange(
                await Task.WhenAll(
                    g.AddOns.Select(async item => new AddOnsLst
                    {
        ...
        public async Task DeleteAddOnFromGroup(Guid ProductId, Guid GroupId)
        {
            ...
            var addOn =
                await _addOnRepository.GetById(product.TypeId)
                ?? throw new NotFoundException(nameof(AddOn), product.TypeId);

            if (addOn.GroupAddOnId != null && addOn.GroupAddOnId != GroupId)
                throw new BadRequestExeption("Add on not in this group");
        ...
        public async Task AddAddOnInGroup(Guid ProductId, Guid GroupId)
        {
            ...
            if (addOn.GroupAddOnId != null)
                throw new BadRequestExeption($"Addon alredy in group {addOn.GroupAddOnId}");
